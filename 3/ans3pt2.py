...
def minimax(list_A, list_B):
    # your work:
    return min(max(list_A), max(list_B))

print(minimax([2,8,4],[0,2,6]))
...
# ceil rounds a float up to the nearest int
# floor rounds a float down to the nearest int
# floor(-1.5) == -2
# int(-1.5) == -1
...
print(sort_by_last_letter(["tictac", "rhubarb", "zebra"]))
...
# X == Y compares X and Y and returns true if they're equal, else false
...
print([compute_x_easy(a, b) for a in [0,1] for b in [0,1]])
...
print([compute_x_hard(a, b, c) for a in [0,1] for b in [0,1] for c in [0,1]])
